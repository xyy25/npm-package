...
export default class Topo {
    ...
    initPosition() {
        ...
        this.data.forEach((item, i) => {
            item.x = points[i].x - 400;
            item.y = points[i].y - 300;
        })
    }

    //根据节点的个数，生成矩形阵列(即配置节点的摆放位置),
    // 返回的points为节点的定位坐标[{x:..,y:...},...]
    getVertices(n) {
        if (typeof n !== 'number') return;
        let [i, j, k] = [0, 0, 0];
        var points = [];
        while (k < n) {
            points.push({ x: 150 * i, y: 100 * j });
            if (i < 5) {
                i++;
            } else {
                i = 0;
    ...
    drawNodeTitle() {
        ...
        .html(d =>
            `<ruby>名称<rp>(</rp><rt>id</rt><rp>)</rp></ruby>: ${d.id}\n` +
            `<ruby>版本<rp>(</rp><rt>version</rt><rp>)</rp></ruby>: ${d.version}\n` +
            `<ruby>路径<rp>(</rp><rt>path</rt><rp>)</rp></ruby>: ${d.path}\n` +
            `<ruby>依赖<rp>(</rp><rt>dependencies</rt><rp>)</rp></ruby>: ${d.requiring.length}个包`);
