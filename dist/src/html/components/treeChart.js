...
function template(settings) {
    ...
    const zoom = d3
        ...
        .scaleExtent([0.1, 100])
        .on("zoom", () => {
        view.panel.attr("transform", "translate(" +
...
function updateNodes(source, nodes) {
    // 给节点添加id，用于选择集索引
    const node = view.panel.selectAll("g.node").data(nodes, (d) => d.id || (d.id = ++i));
...
function updateLinks(source, links) {
    // 更新数据
    let link = view.panel.selectAll("path.link").data(links, (d) => d.id);
