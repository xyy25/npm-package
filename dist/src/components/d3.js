...
class view {
    ...
    template() {
        ...
        const zoom = d3
            ...
            .on("zoom", () => {
            this.view.attr("transform", "translate(" +
    ...
    updateNodes(source) {
        // 给节点添加id，用于选择集索引
        const node = this.view.selectAll("g.node").data(this.nodes, (d) => d.id || (d.id = ++this.i));
    ...
    updateLinks(source) {
        // 更新数据
        var link = this.view.selectAll("path.link").data(this.links, function (d) {
            return d.id;
        });
